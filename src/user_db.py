...
user_db.py
~~~~~~~~~~
SQLite-хранилище: пользователи, платежи, лог доставки.
...
def _run_migrations(db_path: str = DB_PATH) -> None:
    """
    Идемпотентные ALTER TABLE для существующих БД без delivery_log.
    Добавляет таблицу если её нет (покрывается CREATE IF NOT EXISTS выше,
    но миграция оставлена явно для прозрачности истории схемы).
    """
    with _connect(db_path) as conn:
        tables = {
        ...
        if "delivery_log" not in tables:
            logger.info("Миграция: создание таблицы delivery_log")
            # таблица уже создана в init_db выше — этот блок страховочный
