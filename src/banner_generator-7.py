...
def _create_raw_pdf(data: dict) -> io.BytesIO:
    ...
    def rl_measure(text: str, size: float):
        ...
        # Только ascent — без descender.
        # face.descent отрицательное, поэтому ascent - descent завышает высоту,
        # вызывая преждевременный вертикальный fit и уменьшение шрифта.
        # Используем только ascent — соответствует визуальной высоте как в Pillow.
        h = face.ascent / 1000 * size
        return w, h

    details = _calculate_layout(text_items, safe_w, safe_h, measure_fn=rl_measure)

    # ReportLab: y=0 снизу, поэтому считаем от верха.
    # height == ascent, поэтому baseline = y_cursor - height.
    total_h = sum(d["height"] * 1.2 for d in details)
    ...
    for d in details:
        ...
        # height == ascent, baseline = y_cursor - ascent
        y_pos = y_cursor - d["height"]
