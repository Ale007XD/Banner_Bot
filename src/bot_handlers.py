...
async def display_menu(message, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Отображает главное меню с текущими настройками баннера."""
...
async def generate_preview(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not _config_complete(config):
        await update.message.reply_text(
            "❌ Пожалуйста, заполните все параметры (отмечены 🔴).",
        )
        await display_menu(update.message, context)
        return MAIN_MENU
...
async def generate_pdf_callback(
    ...
    channel_caption = (
        ...
        f"📐 *Размер:* {config['width']}×{config['height']} мм\n"
        f"🎨 *Фон / Текст:* {config['bg_color']} / {config['text_color']}\n"
        f"✒️ *Шрифт:* {config['font']}\n"
