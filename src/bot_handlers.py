...
async def display_menu(message, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if _colors_conflict(config):
        lines.append("⚠️ <b>Цвет фона и текста совпадают!</b>")
...
async def save_color(
    ...
    if _colors_conflict(context.user_data["config"]):
        await update.message.reply_text(
            "⚠️ Цвет фона и цвет текста совпадают — выберите другой цвет."
        )
...
async def generate_pdf_callback(
    ...
    try:
        ...
        await query.edit_message_caption(
            caption=f"Ошибка генерации PDF: {exc}"
        )
