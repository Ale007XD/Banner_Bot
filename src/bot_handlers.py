...
# Максимальная длина одной строки баннера (символов)
...
async def save_text_and_continue(
    ...
    """
    Принимает строку текста от пользователя, прогоняет через санитайзер,
    уведомляет об изменениях и сохраняет в config.
    """
    config = context.user_data["config"]
    raw = update.message.text

    clean, modified = sanitize_and_notify(raw, max_length=_MAX_LINE_LENGTH)

    if not clean:
        await update.message.reply_text(
            "⚠️ Текст пустой после очистки — попробуйте другой вариант."
        )
        next_num = len(config.get("text_lines", [])) + 1
        await update.message.reply_text(
            f"Введите текст для строки {next_num}:"
        )
        return AWAIT_TEXT_LINES

    if modified:
        await update.message.reply_text(
            "⚠️ Текст автоматически очищен для печати "
            "(эмодзи и неподдерживаемые символы удалены)."
        )

    config.setdefault("text_lines", []).append(
        {"text": clean, "scale": 1.0}
    )
...
async def save_edited_text(
    ...
    """
    Сохраняет отредактированную строку. Санитайзер применяется так же,
    как при первичном вводе.
    """
    ...
    if not clean:
        ...
        context.user_data["edit_line_index"] = idx
        await update.message.reply_text(
            f"Введите новый текст для строки {idx + 1}:"
        )
        return AWAIT_NEW_TEXT
    ...
    context.user_data["config"]["text_lines"][idx]["text"] = clean
    await display_menu(update.message, context)
    return MAIN_MENU
...
async def generate_preview(
    ...
    # Воронка: превью показано
    record_funnel_event(
        tg_id=update.effective_user.id,
        event=FUNNEL_PREVIEW_GENERATED,
    )
...
async def generate_pdf_callback(
    ...
    # Воронка: инвойс выставлен
    record_funnel_event(
        tg_id=user.id,
        event=FUNNEL_INVOICE_SENT,
        order_number=order_number,
    )
