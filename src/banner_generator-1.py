...
def _create_raw_pdf(data: dict) -> io.BytesIO:
    ...
    w_pt = width_mm * mm
    h_pt = height_mm * mm
    # Layout считается в мм (= px при scale=1) — идентично create_preview_jpeg.
    # rl_measure возвращает ширину в pt → делим на mm для перевода в мм.
    # Высота — из Pillow textbbox (px ≈ мм при scale=1).
    # После layout font_size в мм переводим в pt для ReportLab: * mm.
    ...
    def rl_measure(text: str, size: float):
        # size здесь в мм (единицы layout). Для ReportLab stringWidth нужен pt.
        size_pt = size * mm
        # Ширина в pt → переводим в мм, чтобы единицы совпадали с safe_w_mm.
        w_mm = pdfmetrics.stringWidth(text, font_name, size_pt) / mm
        # Высота — через Pillow textbbox (px ≈ мм при scale=1).
        # face.ascent / 1000 * size завышал высоту → преждевременный fit → мелкий шрифт.
    ...
    # height из layout в мм (Pillow px) → переводим в pt для позиционирования.
    for d in details:
        d["font_size_pt"] = d["font_size"] * mm
        d["height_pt"] = d["height"] * mm

    # ReportLab: y=0 снизу. Центрируем блок вертикально.
    total_h_pt = sum(d["height_pt"] * 1.2 for d in details)
    y_cursor = (h_pt + total_h_pt) / 2  # верхняя граница блока

    for d in details:
        size_pt = d["font_size_pt"]
        text_w = pdfmetrics.stringWidth(d["text"], font_name, size_pt)
        x = (SAFE_ZONE_MM * mm) + ((safe_w_mm * mm) - text_w) / 2
