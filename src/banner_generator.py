...
Ключевое решение UnsupportedDeviceColorSpace:
    OutputIntent с ICC-профилем встраивается уже на этапе ReportLab
    (через низкоуровневый PDF-объект), а не только через Ghostscript.
    Это гарантирует, что GS сохраняет OutputIntent при PDF/A-1b конвертации
    и все DeviceCMYK-объекты на странице имеют корректную привязку к профилю.
"""
...
# ---------------------------------------------------------------------------
# Встраивание OutputIntent в PDF через низкоуровневый API ReportLab
# ---------------------------------------------------------------------------
def _embed_output_intent(c: canvas.Canvas, icc_path: str) -> None:
    """
    Встраивает OutputIntent с ICC-профилем прямо в сырой PDF от ReportLab.

    PDF/A-1b требует, чтобы все Device color spaces на странице имели
    соответствующий OutputIntent. Если его нет — валидатор выдаёт
    UnsupportedDeviceColorSpace.

    Ghostscript при PDF/A-конвертации может не добавить OutputIntent
    корректно если входной PDF его не содержит. Встраивание на этапе
    ReportLab гарантирует наличие OutputIntent во входном файле для GS.

    Структура согласно ISO 19005-1 (PDF/A-1), секция 6.2.2:
        OutputIntents array → OutputIntent dict → DestOutputProfile stream
    """
    try:
        with open(icc_path, "rb") as f:
            icc_data = f.read()
    except OSError as exc:
        logger.warning("Не удалось прочитать ICC-профиль для OutputIntent: %s", exc)
        return

    # Количество компонент CMYK = 4
    icc_stream = c._doc.addStream(
        icc_data,
        {
            "N": 4,
    ...
    output_intent = c._doc.objectReference(
        {
            "Type": "/OutputIntent",
            "S": "/GTS_PDFA1",                     # PDF/A OutputIntent subtype
            "OutputConditionIdentifier": "(ISOcoated_v2_300_eci)",
            "Info": "(ISOcoated v2 300% (ECI))",
            "DestOutputProfile": icc_stream,
        }
    )

    # Добавляем OutputIntents в каталог документа
    c._doc.Catalog.update({"OutputIntents": f"[{output_intent}]"})
    logger.debug("OutputIntent встроен в PDF через ReportLab")
...
#
# Три ключевых требования PDF/A-1b которые закрывает эта функция:
#   1. XMP-метаданные с декларацией соответствия
#   2. OutputIntent с ICC-профилем (решает UnsupportedDeviceColorSpace)
#   3. Шрифты в кривые (dNoOutputFonts) — требование типографий
# ---------------------------------------------------------------------------
def _ghostscript_process(input_path: str, output_path: str) -> None:
    """
    Конвертирует PDF в PDF/A-1b через Ghostscript.

    Исправляет все три ошибки валидации:
      - MissingXMPMetadata         → -dPDFA + -dPDFACompatibilityPolicy
      - UnsupportedDeviceColorSpace → OutputIntent встроен в raw PDF (ReportLab)
                                      + -sColorConversionStrategy=CMYK
      - MissingMarkInfoDictionary  → снято выбором уровня B вместо A

    Убран -dNOSAFER: он блокирует чтение ICC-файлов при некоторых версиях GS,
    что приводит к тому что OutputIntent не записывается в выходной PDF.
    """
    ...
    cmd = [
        ...
        "-dBATCH",
        "-dNOPAUSE",
        # -dNOSAFER намеренно убран: он может блокировать чтение ICC-профилей
        # при конвертации, в результате чего GS не записывает OutputIntent.
        # Без него GS работает в режиме по умолчанию — достаточно для
        # конвертации в tmpdir без доступа к сети.
        "-sDEVICE=pdfwrite",

        # PDF/A-1b: уровень B не требует MarkInfo (Tagged PDF)
        "-dPDFA=1",
        ...
        # CMYK: конвертируем все цвета включая изображения
        "-sColorConversionStrategy=CMYK",
    ...
    if icc_exists:
        cmd += [
            # OutputICCProfile — задаёт целевой профиль для цветовой конвертации
            f"-sOutputICCProfile={ICC_PROFILE_PATH}",
            # DefaultCMYKProfile — профиль для интерпретации DeviceCMYK в источнике
